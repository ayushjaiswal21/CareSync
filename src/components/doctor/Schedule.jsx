...
const Schedule = () => {
  const { user } = useAuth();
  const { appointments, updateAppointmentStatus, cancelAppointment } = useAppointments();
  ...
  const AppointmentCard = ({ apt }) => (
    <div className="bg-white rounded-lg shadow-md overflow-hidden relative">
      ...
      <div className="p-6">
        <h3 className="text-2xl font-bold text-gray-700 mb-4">Patient: {apt.patient?.name || 'N/A'}</h3>
  ...
  return (
    <div className="p-8 bg-gradient-to-b from-purple-50 to-purple-100 rounded-xl shadow-lg">
      <header className="mb-8 text-center">
        <h2 className="text-4xl font-extrabold text-gray-800">Your Appointment Schedule</h2>
      </header>
